...
CREATE TABLE IF NOT EXISTS Remarks(meet_date VARCHAR(10) REFERENCES Reports(meet_date) ON DELETE CASCADE,proctor_id VARCHAR(320) REFERENCES Faculty(faculty_id) ON DELETE CASCADE, student_usn VARCHAR(10) REFERENCES Student(student_usn) ON DELETE CASCADE, remark VARCHAR(250), PRIMARY KEY(meet_date,proctor_id,student_usn));
